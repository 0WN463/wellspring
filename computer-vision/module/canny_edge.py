...
def canny_edge(image, high_threshold, low_threshold, kernel_size=3):
    def make_dog(orientation): return dog(kernel_size, orientation=orientation)

    mag, theta = compute_gradient(image, gradient_kernel=make_dog)

    high_pixels = nonmaximum_suppression(
        np.where(mag >= high_threshold, mag, 0), theta)
    low_pixels = nonmaximum_suppression(
        np.where(mag >= low_threshold, mag, 0), theta)
...
def nonmaximum_suppression(mag, theta, interpolate=True):
    ...
    for i in range(1, height - 1):
        for j in range(1, width - 1):
            ...
            if interpolate:
                target_mag = excess_ratio[i][j] * \
                    next_mag + (1-excess_ratio[i][j]) * main_mag
            else:
...
def hysteresis_threshold(high_pixels, low_pixels):
    ...
    def neighbours(i, j):
        ...
        arr = tuple(filter(lambda e: e[0] >= 0 and e[0]
                    < height and e[1] >= 0 and e[1] < width, arr))
